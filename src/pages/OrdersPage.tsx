    ...
    <div className="page-stack">
      ...
      <section className="panel-card orders-panel">
        {loading ? <Spinner label="Cargando pedidos…" /> : error ? <ErrorState message={error} onRetry={reload} /> : !orders?.length ? (
          ...
          <div className="table-wrap"><table><thead><tr><th>Código</th><th>Fecha</th><th>Cliente</th><th>Total</th><th>Estado</th><th><span className="sr-only">Acciones</span></th></tr></thead>
            <tbody>{orders.map((order) => <tr key={order.id}><td><strong>{order.codigo}</strong></td><td>{formatDate(order.created_at)}</td><td>{order.nombre} {order.apellido}</td><td>{formatCurrency(Number(order.total))}</td><td><StatusBadge status={order.estado} /></td><td className="actions-cell"><button className="button button-secondary button-small" onClick={() => setSelectedOrder(order)}>Ver detalle</button></td></tr>)}</tbody></table></div>
        )}
      </section>
